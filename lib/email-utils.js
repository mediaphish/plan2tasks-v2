...
export async function sendReauthEmail(userEmail, plannerEmail, bypassRateLimit = false) {
  try {
    ...
    console.log(`[EMAIL] Attempting to send re-auth email to ${userEmail} from ${fromEmail}`);
    ...
    // Resend API returns id in data.id or id field
    const emailId = responseData?.data?.id || responseData?.id || null;

    if (emailResponse.ok && emailId) {
      // Update last_reauth_email_sent_at in database
      await updateLastEmailSent(userEmail, plannerEmail);
      console.log(`[EMAIL] Re-authorization email sent successfully to ${userEmail}, Resend ID: ${emailId}`);
      return { sent: true };
    } else {
      const errorMessage = responseData?.message || responseData?.error || responseText || 'Unknown error';
      console.error(`[EMAIL] Failed to send email to ${userEmail}`);
      console.error(`[EMAIL] Response status: ${emailResponse.status}`);
      console.error(`[EMAIL] Response body:`, JSON.stringify(responseData, null, 2));
      console.error(`[EMAIL] Full response text:`, responseText);
      return { sent: false, reason: `Email service error: ${errorMessage}` };
    }
