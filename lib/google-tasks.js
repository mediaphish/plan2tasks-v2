...
export async function getAccessTokenForUser(userEmail) {
  ...
  if (!access || !expiresAt || expiresAt < now + 60) {
    if (!refresh) {
      console.error(`[GOOGLE_TASKS] No refresh token for ${userEmail} - connection needs re-authorization`);
    ...
    try {
      ...
      await sb
        ...
        .eq("user_email", userEmail);
      console.log(`[GOOGLE_TASKS] Token refreshed successfully for ${userEmail}`);
    } catch (refreshError) {
      console.error(`[GOOGLE_TASKS] Token refresh failed for ${userEmail}:`, refreshError);
      // If refresh fails, the connection is broken - user needs to re-authorize
      throw new Error("Token refresh failed - connection needs re-authorization");
    }
  }
